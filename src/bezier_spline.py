...
class BezierCurve:
    def __init__(self, nnodes:int=4) -> None:
        self.nodes = [Point() for _ in range(nnodes)]
        self.nnodes = nnodes
        self.fixed_nodes = {}
        self.binded_nodes = {}
        self.bincoeff = [math.comb(nnodes-1, i) for i in range(nnodes)]
    ...
    def X(self, t:float) -> float:
        ...
        for i in range(self.nnodes):
            X_t += self.bincoeff[i] * ((1-t)**(self.nnodes-1-i)) * (t**i) * self.nodes[i].x
        return X_t
    ...
    def Y(self, t:float) -> float:
        ...
        Y_t = 0.
        for i in range(self.nnodes):
            Y_t += self.bincoeff[i] * ((1-t)**(self.nnodes-1-i)) * (t**i) * self.nodes[i].y
        return Y_t
    ...
    def get_chromo_length(self) -> int:
        return (self.nnodes*2) - len(self.fixed_nodes.keys()) - len(self.binded_nodes.keys())
    ...
    def isvalid(self, S:Dataset) -> bool:
        for nodeidx in range(1, self.nnodes):
            if self.getx(nodeidx) > self.getx(nodeidx+1): return False
        return True
    ...
    def fitness(self, S:Dataset) -> float:
        ...
        sse = 0.
        for dp in S.data:
            if dp.x >= self.getx(1) and dp.x <= self.getx(self.nnodes):
                sse += (self.Y(self.X_inv(dp.x)) - dp.y) ** 2
        
        for (l,u,sign,th) in S.knowledge.sign[0]:
            for x in np.linspace(l, u, 10):
                resid = (self.Y(self.X_inv(x)) - th)
                if sign == '+' and resid < 0: sse += resid ** 2
                elif sign == '-' and resid > 0: sse += resid ** 2
    ...
    def plot(self):
        ...
        plt.plot([self.getx(self.nnodes-1), self.getx(self.nnodes)], [self.gety(self.nnodes-1), self.gety(self.nnodes)], 'ro', linestyle="--")
    

class AnchoredBezierCurve(BezierCurve):
    def __init__(self, xl:float, xu:float, nnodes:int=4) -> None:
        super().__init__(nnodes)
        self.xl = xl
        self.xu = xu

        ## fix nodex w.r.t. x coordinate
        """node_idx = 1
        for xfix in np.linspace(self.xl, self.xu, self.nnodes).tolist():
            self.fixnode(node_idx, 'x', xfix)
            print(f"Fixing node {node_idx} to {xfix}")
            node_idx += 1"""
        self.fixnode(1, 'x', self.xl)
        self.fixnode(self.nnodes, 'x', self.xu)
        
        ## tabulate X_inv
        self.X_inv_tab_size = 1000  # TODO: tab size as parameter or infer from data point distances
        self.X_inv_tab = [super(AnchoredBezierCurve, self).X_inv(x) for x in np.linspace(self.xl, self.xu, self.X_inv_tab_size).tolist() ]
    
    """def X_inv(self, x:float) -> float:
        if x < self.xl or x > self.xu:
            raise RuntimeError('Input x out of domain.')
        
        if x == self.xu: return self.X_inv_tab[ self.X_inv_tab_size - 1 ]
        return self.X_inv_tab[ int( ((x - self.xl) / (self.xu - self.xl)) * self.X_inv_tab_size) ]"""
...
class BezierCurveConnector:
    ...
    def set_chromo(self, chromo:list):
        ...
        for i in range(1, n_curves):
            ...
            p2_t = math.sqrt( (pm1.x-p1.x)**2 + (pm1.y-p1.y)**2 )
