...
class TaylorSplineSolver:
    ...
    def solve_qp(self, silent:bool=False) -> dict:
        ...
        for d in self.S.knowledge.sign.keys():
            ...
            for (_l,_u,s) in self.S.knowledge.sign[d]:
                ...
                for x_sample_idx in range(100):  # TODO: n sample as hyper-parameter
                    x_sample = l
    ...
    def __build_fitexpr_matrix(self, unknowns:list, silent:bool=True) -> (list, list):
        ...
        for dp in self.S.data:
            ...
            kernel_scale = 1. # TODO: remove it in order to use the Kernel
        ...
        #
        # known derivatives (from prior knowledge)
        #
        for deg in self.S.knowledge.derivs.keys():
            for dp in self.S.knowledge.derivs[deg]:
                pass
                """
                kernel_scale = TaylorSplineSolver.Kernel(dp, self.tspline) * 10.
                if kernel_scale == 0.: continue
                ...
                """kernel_scale = TaylorSplineSolver.Kernel(dp, self.tspline)
                if kernel_scale == 0.: continue
                print(f"DP: {dp.x},{dp.y} - {kernel_scale} [{self.tspline.xl}, {self.tspline.xu}]")

                M_row = []
                for n in range(degree):
                    M_row.append( (1. * kernel_scale) if deg == n else 0. )
                M.append(M_row)
    ...
    def PK_Kernel(dp: dataset.DataPoint, tspline: TaylorSpline) -> float:
        if dp.x < tspline.xl or dp.x > tspline.xu: return 0.
        ...
        # ~ Eiffel kernel
        if u == 0: return inf
        return -2.302 * math.log(u_abs)  # TODO: fix coefficient
...
class TaylorSplineEstimator:
    ...
    def fit_fromto(self, S:dataset.Dataset, spline_degree:int, silent:bool=True,
            x0:float=0.0, xl:float=-1., xu:float=1.) -> TaylorSpline:
        
        #
        # fit spline
        # TODO: spline_degree+1 (check it!)
        tspline = TaylorSpline(x0, spline_degree, xl, xu)
        tspline.intersect(S.knowledge.derivs, side='all')
        tspline.fit(S, silent=silent)
